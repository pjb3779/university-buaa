...
void print_err_list() {
	...
	int checkerrD[1000] = {0,};
	int checkerrD_top = 0;
	
	for (int i = 0; i < err_top; i++) {
		if(error_list[i].err_type == 'd'){
			checkerrD[checkerrD_top] = error_list[i].linenum;
			checkerrD_top++;
		}
	}
    ...
    for (int i = 0; i < err_top; i++) {
		...
		if(checkerrD_top != 0 && ErrDindex <= checkerrD_top){
			isErrD = checkerrD[ErrDindex];
			ErrDindex++; 
		}
