...
void parseErrorC(string name) {
    ...
    for (int i = ST_top; i >= 0; i--) {
//        if (symbolTable[i].name == name && 
//		(symbolTable[i].scope == 1 || symbolTable[i].scope == glb_SS || 
//		symbolTable[i].scope == funcStartScope)) {
//            isFound = true;
//            break;
//        }
...
void findAndSetSymbol(const string& tokenName) {
    for (int i = ST_top; i >= 0; i--) { // 심볼 테이블을 역순으로 검색
		...
		(symbolTable[i].scope == 1 || (symbolTable[i].scope >= funcStartScope && symbolTable[i].scope <= glb_SS)) ) {
