...
void parseConstInitVal(int arrexp);
void parseVarDecl();
void parseVarDef(int type, int kind);
void parseInitVal(int arrexp);
void parseFuncDef();
...
vector<Label*> forbreak; 
vector<Label*> forcontinue;
...
class Code{
	...
	public:
		...
		Code(string name, int scope, int addr) : name(name), scope(scope), addr(addr), type(5){}
...
SymbolTable* parseErrorC(string name);
...
void add_s_Table(string name, int type, int kind, int isconst){
	...
	symbolTable[ST_top].pcodearr = PcodeAddress;
	symbolTable[ST_top].arrsize = savearrexp;
	if(isglbscope){
...
void init() {
    ...
    outputFile3.open("pcoderesult.txt");
    if (!outputFile3.is_open()) {
        cerr << "can't open pcoderesult file" << endl;
        return;
    }
		...
		for(int i = 0; i < str.length(); i++){
			...
			if(str[i] == '\\'){
				switch (str[i + 1]) {
					...
					default: val = curToken.word[1]; break; 
				}
			}
			
			Code code3("LDC", val);	//curToken.word[i]
			Code code4("STOS");
			codelist.push_back(code3);
			codelist.push_back(code4);
			arrexp--;
...
void parseVarDef(int type, int kind) {
	...
	if(curToken.word == "="){	//배열 선언 + 초기화 
		...
		for(int i = 0; i < cnt; i++){
			...
			Code code2("LDA", 0, PcodeAddress);
			PcodeAddress++;
			Code code3("LDC", 0);	//배열 값초기화 X 0으로 초기화
			Code code4("STOS");
			codelist.push_back(code1);
			codelist.push_back(code2);
			codelist.push_back(code3);
...
void parseFuncDef() {
	...
	funcInfo[funcInfo_top].StartCode = startedCode;
	...
	if(!isneedreturn){	//if func is void	return 여러게여도 상관없지 않나?
		Code code2("RET");
		codelist.push_back(code2);
	}
...
void parsefor() {
	...
	Label *returnforlabel = new Label();//두번째 위치로 가야 반복문 수행 
	Label *startstmtlabel = new Label();
	Label *returnthree = new Label();

	bool NotCalledCond = true;
	bool isthree = false;
	isFor++;

	forbreak.push_back(endforlabel);
	forcontinue.push_back(returnforlabel);
	...
	returnforlabel->setAdd(codelist.size());
	...
	if(curToken.word != ";"){
		...
		Code codebreak("BZT", endforlabel);
		codelist.push_back(codebreak);
	}
	
	printcurToken();//	두번째;
	nextToken();
	
	returnthree->setAdd(codelist.size() + 1);
	
	if(curToken.word != ")"){
		forcontinue.pop_back();
		forcontinue.push_back(returnthree);

		isthree = true;
		Code codethree("J", startstmtlabel);
		codelist.push_back(codethree);
		
		parseForStmt();
		
		Code codethree2("J",returnforlabel);
		codelist.push_back(codethree2);
	}
	
	printcurToken();//	)
	nextToken();
	
	startstmtlabel->setAdd(codelist.size());
	parseStmt();
	
	if(isthree){
		Code codetothree("J", returnthree);
		codelist.push_back(codetothree);
	}
	
	Code codereturnfor("J", returnforlabel);
	codelist.push_back(codereturnfor);

	isFor--;
	endforlabel->setAdd(codelist.size());
...
void parseStmt() {
	...
	if(curToken.word == "if") {// 'if' '(' Cond ')' Stmt [ 'else' Stmt ] // j
		...
		if(curToken.word == "else") {
			...
			label1->setAdd(codelist.size());

			parseStmt();

			label2->setAdd(codelist.size());
		}else {
			label1->setAdd(codelist.size());
		}
	...
	//'break' ';' | 'continue' ';' // 
		...
		if(curToken.word == "break" && isFor > 0){
			Code code1("J", forbreak.back());
			codelist.push_back(code1);
		}

		if(curToken.word == "continue" && isFor > 0){
			Code code2("J", forcontinue.back());
			codelist.push_back(code2);
...
//		if(expcnt != 0){
	...
	//LVal + exp
		...
		if(isLVal){
			...
			else if(curToken.word == "getchar"){
				Code code1("GETC");
				codelist.push_back(code1);
				
				printcurToken();//getchar
				nextToken();
		...
		//[Exp] ';' // i
			...
			if(curToken.Token == "IDENFR"){
				SymbolTable *symboltemp = parseErrorC(curToken.word);
...
int parsePrimaryExp(int type) {
	 if (curToken.word == "(") {//exp
		...
		int val = curToken.word[1];
		if(curToken.word[1] == '\\'){
			switch (curToken.word[2]) {
				case 'n': val = '\n'; break;
				case 't': val = '\t'; break;
				case 'v': val = '\v'; break;
				case '\\': val = '\\'; break;
				...
				case '0': val = '\0'; break;
				case 'f': val = '\f'; break;
				default: val = curToken.word[1]; break; // 알 수 없는 경우 기본값 설정
			}
		}
		Code code1("LDC", val);
		codelist.push_back(code1);
...
//		isfuncparam = true;    //함수 매개변수 받기 
		...
		Code code2("DOWN", 3 + paramcnt);
		codelist.push_back(code2);
		Code code3("CAL", funcparse.back().StartCode);
		codelist.push_back(code3);
	...
	public:
		vector<string> inter(){
			...
			while(at < codelist.size()){
				...
				else if (curCode.getName() == "MOD128")
				{
				 	dstack[sp] = dstack[sp] % 128;
				 	at++;
				}
				...
				else if (curCode.getName() == "GETC")
				{	
					sp++;
					char input;
					cin >> input;
					dstack[sp] = static_cast<int>(input);
					at++;
				}
				else if (curCode.getName() == "PRF")
				{
					...
					// 스택 포인터를 포맷 지정자의 개수만큼 감소시킵니다.
					sp = sp - cnt;
					// "%d"와 "%c"를 찾아서 해당 값을 숫자 또는 문자로 치환합니다.
					for (int i = 0; i < cnt; i++) {
						size_t pos = s.find('%');
						if (pos != string::npos) {
							// 포맷 지정자의 다음 문자를 확인합니다.
							if (s[pos + 1] == 'd') {
								s.replace(pos, 2, to_string(dstack[sp + i + 1]));
							} else if (s[pos + 1] == 'c') {
								
								char ch = static_cast<char>(dstack[sp + i + 1] % 128);//char범위 맞춰 128나누기 
								s.replace(pos, 2, string(1, ch));
							}
						}
					}
				...
				cout << "sp == " << sp << "  " << "sp value == " << dstack[sp] << "  baddr == " << BAddr << "\n\n";
			}
			return print;
		}
		void outputPCode(vector<string> interprint)
		{
			...
			{
				cout << interprint[i];
				outputFile3 << interprint[i];
			}
		}
		
		void outputDstack(){
			for (int i = 0; i < sp; i++){
				cout << "sp = " << i << " value = " << dstack[i] << endl;
			}
		}
};
