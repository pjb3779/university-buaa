...
bool scopeeqaul = false;	//함수 시작 스코프와 같으면 
...
struct funcparam{
	...
	string name;
	int paramcnt = 0;
	int erreparamcur = 0; 	//함수 매개변수 위치 
	vector<string> paramtype;
};
funcparam funcInfo[100001];
vector<funcparam> ErrEtemp;
funcparam debug[100001];
int debug_top = 0;
int param_func_cur = 0;	//함수 매개변수로 함수가 들어올때 변수 위치 
int funcInfo_top = 0;
...
void parseUnaryExp() {
	...
	if(curToken.word == "+" || curToken.word == "-" || curToken.word == "!"){
		...
		if(!parseErrorJ()){		//에러j가 우선순위를 가짐 
			parseErrorD(identname, 	ErrEtemp[param_func_cur - 1].erreparamcur, last_func_line);
		}
		
		//함수 매개변수 확인중
//		debug[debug_top].type = ErrEtemp[ErrEtemp.size() - 1].type;
//		debug[debug_top].name = ErrEtemp[ErrEtemp.size() - 1].name;
//		debug[debug_top].paramcnt = ErrEtemp[ErrEtemp.size() - 1].paramcnt;
//		debug[debug_top].erreparamcur = ErrEtemp[ErrEtemp.size() - 1].erreparamcur;
//		debug_top++;
		
		ErrEtemp.pop_back();
		param_func_cur--;		//매개변수로 선언된 함수처리 
...
void parseFuncRParams() {
	...
	if(isfuncparam){	//errorD처리중 
		ErrEtemp[param_func_cur - 1].erreparamcur++;
	}
...
void parseErrorE() {
	if(!isfuncDef){  // 함수 선언인지 확인
		if(isdigit(curToken.word[0])){  // 매개변수가 숫자인 경우
			if(param_func_cur - 1 < ErrEtemp.size() && ErrEtemp[param_func_cur - 1].erreparamcur < ErrEtemp[param_func_cur - 1].paramtype.size()) {
				if(ErrEtemp[param_func_cur - 1].paramtype[ErrEtemp[param_func_cur - 1].erreparamcur] != "Int"){
					push_err_list('e', last_func_line);
				}
			} else {
				push_err_list('e', last_func_line);  // 범위 초과 시 오류 처리
			}
			return; 
		}
		...
		for(int i = ST_top - 1; i >= 0 ; i--){
			if(curToken.word == symbolTable[i].name){
				...
				// Func 제거
				pos = identtype.find("Func");
				if(pos != string::npos){
					identtype.erase(pos, 4);
        			while(identtype.front() == ' ') {
            			identtype.erase(identtype.begin());
       				}
        			while(identtype.back() == ' ') {
				...
				// 벡터 범위 검사 후 매개변수 처리
				if(param_func_cur - 1 < ErrEtemp.size() && ErrEtemp[param_func_cur - 1].erreparamcur < ErrEtemp[param_func_cur - 1].paramtype.size()) {
					if(identtype != ErrEtemp[param_func_cur - 1].paramtype[ErrEtemp[param_func_cur - 1].erreparamcur]){
						push_err_list('e', last_func_line);
					}
				} else {
					push_err_list('e', last_func_line);  // 범위 초과 시 오류 처리
...
//	함수저장 출력 
	...
	//erre 출력
	cout << "\n\n";
	for(int i = 0; i < debug_top ; i++){
		cout << debug[i].type << " " << debug[i].name << " " << debug[i].paramcnt << " " << 
		debug[i].erreparamcur << endl; 
	}
