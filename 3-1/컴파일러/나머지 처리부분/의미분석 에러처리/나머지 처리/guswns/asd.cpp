	...
	if(curToken.word == "+" || curToken.word == "-" || curToken.word == "!"){
    	...
    	if(curToken.word != ")"){
    		if(curToken.word != ";"){// 그냥 )parse err 
    			errdcnt = parseFuncRParams();
			}
		}
		
		if(!parseErrorJ()){		//에러j가 우선순위를 가짐 
			if(errdcnt != 0){
				parseErrorD(identname, 	errdcnt, last_func_line);
			}
		}
...
//FuncRParams → Exp { ',' Exp } 
int parseFuncRParams() {
	int paramscnt = 0;	//errorD 매개변수 갯수 초기화 
...
void parseErrorE(int param_num) {
	if (funcparse_top <= 0) {//0일떄 1로 변경 배열 오류 방지 근데 왜? 
		funcparse_top = 1;
		cerr << "Error: funcparse_top is invalid." << endl;
        return;
	}
		
	funcparam& curFunc = funcparse[funcparse_top - 1];
	
//	printfSymbol[print_top].name = curFunc.name + "start" + (curFunc.paramcnt);	//print only
//	printfSymbol[print_top].type = curFunc.type;
//	print_top++;
	
	if(curFunc.paramcnt == 0){//매개변수 없을경우 체크 x 
		return;
	}
	//변수일때 
	if(curToken.Token == "IDENFR"){
		...
		} else if(curFunc.paramtype[param_num - 1] == 0){//일반 변수
