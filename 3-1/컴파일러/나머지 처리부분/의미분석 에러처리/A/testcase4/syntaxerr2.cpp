...
bool parseErrorJ();
void parseErrorK();
void parseErrorL(string formatString, int print_param_cnt);
void parseErrorM();
void print_err_list();
...
//		//이거는 문제를 쫌 잘못낸거 같은데? 함수 선언시 닫는 괄호가 없을떄 바로 블럭파싱 
//		if(curToken.word != "{"){	 
//			parseFuncFParams();
//		}
//	}
...
void parseErrorE() {
	if(curToken.Token != "IDENFR"){		//함수 선언부 제외 
