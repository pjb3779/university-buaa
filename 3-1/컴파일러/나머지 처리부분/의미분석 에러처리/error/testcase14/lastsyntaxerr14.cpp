...
void print_err_list() {
    // 최대 에러 수만큼의 배열 초기화
    int printed_line[100001] = {0};  // 이미 출력된 줄 번호를 저장할 배열
    char printed_type[100001] = {0}; // 이미 출력된 에러 유형을 저장할 배열
    ...
    for (int i = 0; i < err_top; i++) {
        ...
        for (int j = 0; j < i; j++) {
            if (printed_line[j] == line_num && printed_type[j] == err_type) {
                is_duplicate = true;
                break;
            }
        }
        ...
        // 출력된 에러 기록
        printed_line[i] = line_num;
        printed_type[i] = err_type;
    }
}
...
void parseErrorE(int param_num) {
	...
	if(curToken.Token == "IDENFR"){
		...
		if(curFunc.paramkind[param_num] == 3){//배열일떄  
			if(SymbolTemp.type != curFunc.paramtype[param_num] || SymbolTemp.kind != curFunc.paramkind[param_num]){
				push_err_list('e');
			}
		} else if(SymbolTemp.type != curFunc.type){//일반 변수 또는 함수 
			push_err_list('e');
		}
	} else if(isdigit(curToken.word[0])){//숫자일떄 
		if(curFunc.type != 0){
			push_err_list('e');
		} 
	}

	int curType = parseExp();
