...
#define BASE_ADDR ((uint32_t)0x00003000)
#define BASE_DATA ((uint32_t)0x00000000)
...
static int write_machine_code(FILE *input, FILE *output, SymbolTable *symtbl, SymbolTable *reltbl)
{
    ...
    while (fgets(buf, LINKER_BUF_SIZE, input))
    {
        ...
        {
            int32_t res = relocate_inst(inst, offset, symtbl, reltbl);
            if (res == -1)
            {
                printf("write_machine_code: relocate instruction %08" PRIx32 " failed!\n", inst);
                return -1;
            }
            inst = res;
        }
